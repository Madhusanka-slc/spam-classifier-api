...
class AIModel:
    ...
    def get_label_legened_inverted(self):
        legened = self.get_metadata().get('labels_legend_inverted') or {}
        if len(legened.keys()) != 2:
            raise Exception("Your legend is incorrect")
        return legened

    def get_label_pred(self, idx, val):
        legened = self.get_label_legened_inverted()
        return {"label": legened[str(idx)], "confidence": val}
