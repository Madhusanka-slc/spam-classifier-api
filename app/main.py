...
@app.post("/")
def create_inferece(query: schema.Query):
    global AI_MODEL
    ...
    # NoSQL -> cassandra -> DataStax AstraaDB
    return obj

@app.get("/")
def read_index(q: Optional[str] = None):
    return {"message": "Welcome to Spam SMS Classifier API"}
...
def fetch_rows(stmt:SimpleStatement, fetch_size:int = 2, session=None):
...
def export_inferences():
    ...
    return StreamingResponse(fetch_rows(statement,2,DB_SESSION))
